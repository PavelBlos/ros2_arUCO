...
class TagRegistry:
    ...
    def update_defaults(self, marker_size_mm: float, ceiling_z_m: float) -> Tuple[int, str]:
        """Persist physical defaults used for unknown tags and new anchors."""
        marker_size_mm = float(marker_size_mm)
        ceiling_z_m = float(ceiling_z_m)
        if not math.isfinite(marker_size_mm) or not 20.0 <= marker_size_mm <= 1000.0:
            raise ValidationError("default marker size must be between 20 and 1000 mm")
        if not math.isfinite(ceiling_z_m) or not 0.2 <= ceiling_z_m <= 20.0:
            raise ValidationError("ceiling height must be between 0.2 and 20 m")
        with self.lock:
            changed = (
                self.default_marker_size_mm != marker_size_mm
                or float(self._data.get("ceiling_z_m", 2.5)) != ceiling_z_m
            )
            if not changed:
                return self.revision, self.sha256
            self._data["default_marker_size_mm"] = marker_size_mm
            self._data["ceiling_z_m"] = ceiling_z_m
            self._data["revision"] = self.revision + 1
