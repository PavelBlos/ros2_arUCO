...
class VideoTagDetector(Node):
    def __init__(self):
        ...
        self.declare_parameter('detection_rate', 10.0) # Частота обработки кадров (10 Гц)
        ...
        self.get_logger().info(f"Params: video={self.video_path}, calib={self.calibration_path}, marker_size={self.marker_length}m")
    ...
    def init_aruco_detector(self):
        try:
            ...
            self.get_logger().info("Initialized OpenCV 4.7+ ArUco detector (DICT_4X4_50)")
            ...
            self.get_logger().info("Initialized Legacy OpenCV ArUco detector (DICT_4X4_50)")
    ...
    def timer_callback(self):
        ...
        # Если видео закончилось, сбрасываем на начало (циклическое воспроизведение для непрерывного теста)
        if not ret:
            self.get_logger().info("Video finished. Looping back to the beginning.")
            self.cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
            return

        # 1. Устранение дисторсии (искажений линзы) на основе калибровки
        ...
        if ids is not None:
            ...
            for i in range(len(ids)):
                ...
                ret_pnp, rvec, tvec = cv2.solvePnP(obj_pts, corners[i][0], self.camera_matrix, np.zeros(5), flags=cv2.SOLVEPNP_ITERATIVE)
