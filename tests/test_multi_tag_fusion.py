...
def test_frame_metadata_requires_exact_epoch_revision_and_sha(test_setup):
    K, dist, T_base_cam = test_setup
    result = MultiTagFusion().process_frame(
        [], {}, K, dist, T_base_cam, (0.0, 0.0, 0.0),
        expected_epoch=3, expected_revision=7, expected_sha256="abc",
        frame_epoch=2, frame_revision=7, frame_sha256="abc",
    )
    assert result["status"] == "revision_mismatch"
    result = MultiTagFusion().process_frame(
        [], {}, K, dist, T_base_cam, (0.0, 0.0, 0.0),
        expected_epoch=3, expected_revision=7, expected_sha256="abc",
        frame_epoch=3, frame_revision=7, frame_sha256="wrong",
    )
    assert result["status"] == "revision_mismatch"
