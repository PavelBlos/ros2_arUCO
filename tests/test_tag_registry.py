...
def test_physical_defaults_are_versioned_and_validated(tmp_path):
    reg = TagRegistry(str(tmp_path / "tags.yaml"))
    old_revision = reg.revision
    revision, _ = reg.update_defaults(120.0, 2.8)
    assert revision == old_revision + 1
    assert reg.default_marker_size_mm == 120.0
    assert reg._data["ceiling_z_m"] == 2.8
    with pytest.raises(ValidationError):
        reg.update_defaults(5.0, 2.8)
