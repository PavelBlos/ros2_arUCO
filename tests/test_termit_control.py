...
class ControlTests(unittest.TestCase):
    ...
    def test_hardware_api_forward_motion(self):
        # Hardware API keeps its proven convention: vx=right, vy=forward.
    ...
    def test_hardware_api_backward_motion(self):
        self.api.drive(0.0, -0.10, 0.0)
        s1, s2, s3 = self.api._target_steps_locked()
    ...
    def test_hardware_api_left_motion(self):
    ...
    def test_hardware_api_right_motion(self):
        self.api.drive(0.10, 0.0, 0.0)
        s1, s2, s3 = self.api._target_steps_locked()
    ...
    def test_wheel_diameter_update_changes_command_and_odometry_scale(self):
        old_spm = self.api._steps_per_meter
        self.api.set_wheel_diameter_mm(80.0)
        self.assertAlmostEqual(self.api.config.wheel_radius, 0.04)
        self.assertLess(self.api._steps_per_meter, old_spm)
        self.assertAlmostEqual(self.api._meters_per_step, 1.0 / self.api._steps_per_meter)
        with self.assertRaises(ValueError):
            self.api.set_wheel_diameter_mm(5.0)
