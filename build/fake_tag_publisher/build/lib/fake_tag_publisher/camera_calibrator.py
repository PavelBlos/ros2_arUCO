...
class CameraCalibrator(Node):
    ...
    def run_calibration(self):
        ...
        while cap.isOpened():
            ...
            # Обрабатываем каждый N-й кадр, чтобы собрать разнообразные ракурсы без перегрузки памяти
        ...
        if ret:
            ...
            try:
                ...
                # Поднимаемся на 4 уровня вверх из install/fake_tag_publisher/share/fake_tag_publisher до корня ros2_ws
                ws_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(share_dir))))
                src_config_path = os.path.join(ws_root, 'src', 'fake_tag_publisher', 'config', 'camera_info.yaml')
